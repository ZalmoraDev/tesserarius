...
final readonly class ProjectMembersService implements ProjectMembersServiceInterface
{
    ...
    public function getProjectMembersByProjectId(int $projectId): array
    {
        return ServiceException::handleRepoCall(
            fn() => $this->projectMembersRepo->findProjectMembersByProjectId($projectId),
            ProjectMembersException::class,
            __FUNCTION__
        );
    }
    ...
    {
        return ServiceException::handleRepoCall(
            fn() => $this->projectMembersRepo->findProjectInvitesByProjectId($projectId),
            ProjectMembersException::class,
            __FUNCTION__
        );
    }
    ...
    public function promoteProjectMember(int $projectId, int $userId): void
    {
        ServiceException::handleRepoCall(
            fn() => $this->projectMembersRepo->promoteProjectMember($projectId, $userId),
            ProjectMembersException::class,
            __FUNCTION__
        );
    }
    ...
    {
        ServiceException::handleRepoCall(
            fn() => $this->projectMembersRepo->demoteProjectMember($projectId, $userId),
            ProjectMembersException::class,
            __FUNCTION__
        );
    }
    ...
    {
        ServiceException::handleRepoCall(
            fn() => $this->projectMembersRepo->removeProjectMember($projectId, $userId),
            ProjectMembersException::class,
            __FUNCTION__
        );
    }
    ...
    public function generateProjectInviteCodes(int $projectId, DateTimeImmutable $expiresAt, int $count): void
    {
        ...
        for ($i = 0; $i < $count; $i++)
            $invites[] = new ProjectInvite(0, $projectId, $this->generateInviteCode(16), $expiresAt, null, $createdBy, $now);

        ServiceException::handleRepoCall(
            fn() => $this->projectMembersRepo->createProjectInviteCodes($invites),
            ProjectMembersException::class,
            __FUNCTION__
        );
    }
    ...
    public function joinProjectByInviteCode(string $inviteCode): int
    {
        ...
        return ServiceException::handleRepoCall(
            fn() => $this->projectMembersRepo->joinProjectByInviteCode($inviteCode, (int)$_SESSION['auth']['userId']),
            ProjectMembersException::class,
            __FUNCTION__
        );
    }
    ...
    {
        ServiceException::handleRepoCall(
            fn() => $this->projectMembersRepo->deleteProjectInviteCode($projectId, $inviteId),
            ProjectMembersException::class,
            __FUNCTION__
        );
    }
